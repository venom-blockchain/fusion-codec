...
pub fn serialize_transaction(tr: Transaction) -> Result<Value> {
    ...
    let mut map = db_serialize_transaction_ex("id", set, SerializationMode::QServer)?;
    let mut out_msgs: Vec<Map<String, Value>> = Vec::new();

    if let Some(_msg) = tr.in_msg {
        let msg = _msg.read_struct()?;

        let in_msg = db_serialize_message_ex("id", &MessageSerializationSet {
            message: msg.clone(),
            id: msg.hash()?,
            block_id: None,
            transaction_id: None,
            transaction_now: None,
            status: MessageProcessingStatus::Finalized,
            boc: serialize_toc(&_msg.cell())?,
            proof: None,
        }, SerializationMode::QServer)?;

        map.insert("in_msg".to_owned(), in_msg.into());
    }

    tr.out_msgs.iterate_slices(|slice| {
        if let Some(cell) = slice.reference_opt(0) {
            let msg = Message::construct_from_cell(cell.clone())?;

            let msg_map = db_serialize_message_ex("id", &MessageSerializationSet {
                message: msg.clone(),
                id: cell.repr_hash(),
                block_id: None,
                transaction_id: None,
                transaction_now: None,
                status: MessageProcessingStatus::Finalized,
                boc: serialize_toc(&cell)?,
                proof: None,
            }, SerializationMode::QServer)?;

            out_msgs.push(msg_map)
        }

        Ok(true)
    })?;

    map.insert("out_msgs".to_owned(), out_msgs.into());
