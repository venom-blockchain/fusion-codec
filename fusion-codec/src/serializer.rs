...
pub fn parse(abi: &str) -> Result<AbiEntity, JsValue> {
    let abi = abi.trim();
    let custom_abi = fusion_abi_parser::Entity::parse(abi)
        .or_else(
            |e| match serde_json::from_str::<ton_abi::contract::SerdeFunction>(abi) {
                Ok(raw) => {
                    ...
                    Ok(fusion_abi_parser::Entity::Function(function))
                }
                Err(_) => Err(e),
            },
        )
    ...
    Ok(match custom_abi {
        fusion_abi_parser::Entity::Empty => ObjectBuilder::new().set("kind", "empty"),
        fusion_abi_parser::Entity::Cell(params) => ObjectBuilder::new()
            .set("kind", "cell")
            .set("structure", make_params(params)),
        fusion_abi_parser::Entity::Function(function) => ObjectBuilder::new()
            .set("kind", "function")
